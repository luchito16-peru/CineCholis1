...
<script>
	document
			.addEventListener(
					'DOMContentLoaded',
					function() {
						const cardNumberInput = document
								.getElementById('card-number');
						const expiryDateInput = document
								.getElementById('expiry-date');
						const cvvInput = document.getElementById('cvv');
						const visaIcon = document
								.querySelector('img[alt="Visa"]');
						const mastercardIcon = document
								.querySelector('img[alt="Mastercard"]');
						const amexIcon = document
								.querySelector('img[alt="American Express"]');
						const yapeButton = document
								.getElementById('yape-button');
						const yapeForm = document.getElementById('yape-form');
						const cardPaymentForm = document
								.getElementById('card-payment-form');
						const paymentIcons = document
								.querySelector('.payment-icons');
						const paymentHeader = document
								.querySelector('.payment-header');
								...
								.addEventListener(
										...
										function(event) {
											...
											const isAmex = cardNumber
													...
													|| cardNumber
															.startsWith('37');
											if ((!isAmex && cardNumber.length !== 16)
													|| (isAmex && cardNumber.length !== 15)) {
											...
											// Validar CVV
											let cvv = cvvInput.value.replace(
													/\D/g, '');
						...
						yapeButton
								.addEventListener(
										...
										function() {
											...
											document
													.getElementById('card-button').style.display = 'block'; // Mostrar botón de pagar con tarjeta
						...
						cvvInput.addEventListener('input', function() {
							...
							const isAmex = cardNumberInput.value
									.startsWith('34')
									|| cardNumberInput.value.startsWith('37');
							const maxCVVLength = isAmex ? 4 : 3;
	...
	<div class="main-content-pago">
		<div class="main-content-productos">
			<div class="left-panel-productos">
				...
				<p>
					Sala:
					<%=sala != null ? sala : "Sala no disponible"%></p>
				<p>
					Butacas seleccionadas: <strong><%=(asientos != null && !asientos.isEmpty()) ? asientos : "Ninguna"%></strong>
				</p>
			...
			<div class="payment-panel">
				...
				<div class="payment-icons">
					<img src="img/visa.png" alt="Visa" /> <img
						src="img/mastercard.png" alt="Mastercard" /> <img
						src="img/americanexpress.png" alt="American Express" />
				</div>

				<!-- Formulario de Pago con Tarjeta -->
				<form id="card-payment-form" action="logicajsp/logicapago.jsp"
					method="post">
					<input type="hidden" name="asientos" value="<%=asientos%>" /> <input
						type="hidden" name="total"
						value="<%=total != null ? total : "0.00"%>" /> <input
						type="hidden" name="usuario" value="<%=usuario%>" /> <input
						type="hidden" name="cine" value="<%=cine%>" /> <input
						type="hidden" name="sala" value="<%=sala%>" /> <input
						type="hidden" name="pelicula" value="<%=pelicula%>" /> <input
						type="hidden" name="fecha" value="<%=fecha%>" /> <input
						type="hidden" name="horario" value="<%=horario%>" /> <input
						type="hidden" name="forma_pago" value="Tarjeta" />
					...
					<div class="form-group flex-group">
						<input type="text" id="expiry-date" name="expiry-date"
							placeholder="Fecha de vencimiento MM/AA" required /> <input
							type="text" id="cvv" name="cvv" placeholder="CVV" required />
					</div>
					<div class="form-group">
					...
					<p style="font-size: 14px; color: #333;">Estimado usuario, por
						seguridad le enviaremos su comprobante de pago a su correo
						electrónico.</p>
					<div class="terms-total-container">
						<div class="terms-and-conditions">
							<input type="checkbox" id="terms" name="terms" required /> <label
								for="terms">Acepto los términos y condiciones</label>
						</div>
						<p>
							Total: <strong>S/. <%=(total != null && !total.isEmpty()) ? total : "0.00"%></strong>
						</p>
					</div>
				...
				<form id="yape-form" method="post"
					action="logicajsp/logica_yape.jsp" style="display: none;"
					onsubmit="return validarYape();">
					<h2>Pagar con Yape</h2>
					<p>Escanea el código QR para pagar con Yape</p>
					<img src="img/yape.jpeg" alt="Código QR" class="qr-code" />

					<div class="form-group">
						<input type="text" id="celular" name="celular"
							placeholder="Número de celular" required maxlength="14"
							oninput="formatCelular(this)" /> <small style="color: #666;">Ejemplo:
							965 458 658</small>
					</div>
					...
					<div class="form-group">
						<input type="text" name="full-name" placeholder="Nombre Completo"
							required />
						<!-- Campo de nombre completo visible -->
					</div>
					<div class="form-group">
						<input type="email" id="yape-email" name="email"
							value="<%=usuario%>" placeholder="Correo Electrónico" required
							readonly />
					</div>
					<p style="font-size: 14px; color: #333;">Recibirá un
						comprobante de pago por correo.</p>
					...
					<input type="hidden" name="asientos" value="<%=asientos%>" /> <input
						...
						type="hidden" name="fecha" value="<%=fecha%>" /> <input
						type="hidden" name="horario" value="<%=horario%>" /> <input
						type="hidden" name="forma_pago" value="Yape" />


					<button type="submit" class="btn-confirmar">Confirmar Pago</button>
				</form>

				<button id="card-button" class="btn-pagar-targeta"
					style="display: none;">Pagar con Tarjeta</button>
			</div>
		</div>
	</div>
	<script>
		const emailInput = document.getElementById('email');
		const yapeEmailInput = document.getElementById('yape-email');
		...
		if (usuarioGuardado) {
			...
			// Asigna el correo almacenado al input
			emailInput.value = usuarioGuardado;
			yapeEmailInput.value = usuarioGuardado;
		} else {
